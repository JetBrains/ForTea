...
namespace GammaJul.ForTea.Core.Tree.Impl
{
  ...
  {
    [NotNull] private static Regex MacroRegex { get; } = new Regex(@"\$\((\w+)\)", RegexOptions.Compiled);
    ...
    [CanBeNull] private T4ResolvedPath MyResolvedPath { get; set; }
    ...
    [CanBeNull] protected abstract string RawPath { get; }
  }
}