...
namespace GammaJul.ForTea.Core.Psi.Resolve
{
  ...
  {
    [NotNull, ItemCanBeNull] private Stack<IProjectFile> ProjectFiles { get; } = new Stack<IProjectFile>();

    [CanBeNull] public IProjectFile MostSuitableProjectFile => ProjectFiles.Reverse().WhereNotNull().FirstOrDefault();
    ...
    {
      [NotNull] private T4MacroResolveContext Provider { get; }
