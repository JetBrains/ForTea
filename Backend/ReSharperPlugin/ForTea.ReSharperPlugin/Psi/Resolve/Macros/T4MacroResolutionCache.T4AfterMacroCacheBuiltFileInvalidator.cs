...
namespace JetBrains.ForTea.ReSharperPlugin.Psi.Resolve.Macros
{
	...
	public sealed class T4AfterMacroCacheBuiltFileInvalidator
	{
		...
		[NotNull]
		private List<IPsiSourceFile> FilesToInvalidate { get; set; } = new();

		[NotNull]
		private IPsiServices Services { get; }
		...
		private void QueueAfterCommit() => Services.Locks.ExecuteOrQueue(Lifetime, "T4 file invalidation caused by a change in macros in that file", () =>
		{
